...
export function Step1UrlInput({
  ...
  // 영상이 선택(메타 로드)되면 "선택된 영상"으로 명확히 보여준다(검색/직접 입력에서 조용히
  // 채우지 않음). 커머스(상품 URL)는 메타 미리보기가 없어 기존 입력 UI 유지.
  const hasSelectedVideo = !isCommerce && status === "success" && !!videoInfo;
        ...
        {hasSelectedVideo ? (
          <>
            ...
            <p className="mt-2 text-sm font-medium leading-relaxed tracking-ko text-text-muted">
              이 영상으로 카드를 만들어요. 다른 영상으로 바꿀 수 있어요.
            </p>
            ...
            <button
              type="button"
              onClick={() => onChange("")}
              className="mt-3 inline-flex min-h-[44px] items-center gap-1 rounded-lg border border-border bg-white px-4 text-sm font-semibold tracking-ko text-text-strong transition-colors hover:border-text-muted hover:bg-surface"
            >
            ...
            <div className="mt-6 flex gap-2">
              <div className="relative min-w-0 flex-1">
                ...
                <Input
                  ...
                  placeholder={
                    isCommerce ? "https://smartstore.naver.com/..." : "https://youtu.be/..."
                  }
