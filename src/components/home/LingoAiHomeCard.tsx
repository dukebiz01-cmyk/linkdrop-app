 ...
 *
 * 표면 1개: (옵션)오늘 가이드 슬롯 + 디바이더 + 단일 헤더 트리거
 *   "[전구] 링고AI 성과 진단 보기 … [셰브론]"(한 줄 전체 아코디언 토글, 펼침 시점에만 CreatorCoachCard 마운트).
...
export function LingoAiHomeCard({ guideSlot }: { guideSlot?: ReactNode }) {
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-[#E5E5E5] bg-white">
      {/* 상단 — 오늘 가이드(TodayAiCard) 슬롯. 미주입 시 슬롯 영역+디바이더 통째 미렌더
          → 단일 헤더 트리거만 노출(빈 슬롯·빈 디바이더 방지). */}
      {guideSlot ? (
        <>
          {/* 자식 크롬 중화: 배경·패딩·자체 h2 숨김. */}
      ...
      {/* 단일 헤더 트리거 — [전구] 링고AI 성과 진단 보기 … [셰브론]. 한 줄 전체가 아코디언 토글.
          기존 "링고AI" 헤더 라벨을 이 한 줄에 흡수(분리 헤더 제거). */}
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
        className="flex w-full min-h-[52px] items-center justify-between px-4 text-sm font-bold tracking-ko text-[#0A0A0A] transition-colors hover:bg-[#FAFAFA]"
      >
      ...
      {open ? (
        // 펼침 시점 마운트(lazy) — 트리거 아래로 정상 블록 흐름(absolute·음수마진 없음).
        //   코치 자체 크롬(shadow/rounded/p) 중화 + 내부 중복 "링고AI" h2 숨김(트리거와 중복/겹침 방지).
        //   내부 기간 토글은 유지.
        <div className="px-4 pb-4 [&>section]:rounded-none [&>section]:bg-transparent [&>section]:p-0 [&>section]:shadow-none [&_h2]:hidden">
