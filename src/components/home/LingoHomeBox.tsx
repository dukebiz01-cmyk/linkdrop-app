...
export function LingoHomeBox({
  ...
  // LINGO-UI-2 §3-2 ② — 상태 칩(3분기 로직 재사용 · 문구 교체). 학습형 칩은 기존 sendChat 재사용.
  const chips =
    greet === "starter"
      ? [
          { key: "learn", label: "처음부터 배우기", primary: true, onTap: () => void sendChat("링크드롭 사용법을 처음부터 알려주세요") },
          { key: "make", label: "카드 만들러 가기", primary: false, onTap: () => onGoStudio() },
        ]
      : greet === "share"
        ? [
            { key: "how-share", label: "카드 공유하는 법", primary: true, onTap: () => void sendChat("만든 카드를 카톡으로 공유하는 방법을 알려주세요") },
            { key: "new", label: "새 카드 만들기", primary: false, onTap: () => onGoStudio() },
          ]
        : [
