...
// LINGO-UI-2c — 펼침 인사 = 대본 v6 1-0 다섯 줄(한 글자 락).
const GREETING_LINES = [
  "안녕하세요, 링고AI예요. 저는 링크드롭의 안내자이자 어드바이저입니다.",
  "무엇을 도와드릴까요? 링크드롭 사용법, 저와 함께 시작해볼까요?",
  "영상이나 이미지 링크 하나로 재밌고 유용한 정보를 전할 수 있어요.",
  "손님에게 혜택을 주는 쿠폰도, 쉽고 빠른 예약도 붙일 수 있어요.",
  "다양한 상품 판매까지, 여러 템플릿으로 쉽고 간편하게 만들어 드려요.",
];
...
// LINGO-FIX-4b — stripMarkdown 은 공용 승격(src/lib/lingo-text.ts) — import 로 전환(동작 0변화).
        ...
        <div className="border-t border-[#E8EDF3] px-4 pb-4 pt-3">
          ...
          {chat.messages.length === 0 && (
            <div className="space-y-1">
              {/* LINGO-UI-2c — v6 1-0 다섯 줄(한 글자 락). */}
              {GREETING_LINES.map((line) => (
                <p key={line} className="text-[13px] font-medium leading-relaxed text-[#404040] [word-break:keep-all]">
                  {line}
                </p>
              ))}
            </div>
              ...
              {chat.messages.map((m) => (
                <div key={m.id} className={`flex ${m.role === "user" ? "justify-end" : "justify-start"}`}>
                  <p className={`max-w-[85%] whitespace-pre-wrap rounded-2xl px-3 py-2 text-[13px] font-medium leading-relaxed [word-break:keep-all] ${m.role === "user" ? "rounded-br-md text-white" : "rounded-bl-md bg-white text-[#404040] [box-shadow:inset_0_0_0_1px_#ECECEE]"}`} style={m.role === "user" ? { backgroundColor: ACCENT } : undefined}>
                    {stripMarkdown(m.text) || (m.streaming ? <Loader2 className="h-3.5 w-3.5 animate-spin" strokeWidth={2.5} style={{ color: ACCENT }} /> : "")}
                  </p>
                </div>
              ))}
            </div>
