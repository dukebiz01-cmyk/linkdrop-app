/**
 * v2 BlockEditor 기반 만들기 화면 (백업).
 * WHY: /create 는 v3 CreateDropWizard 로 전환됐지만 Supabase 분리 INSERT·블록 편집 로직은
 *      createDropV2 연동 시 참고용으로 보존한다.
 */
...
/** v3 UX 5목적 — DB drop_purpose / intent_types.purpose 와 동일 레이블. */
type DropPurposeUx = "info" | "coupon" | "reservation" | "purchase" | "lead";

type DbDropPurpose = "정보" | "쿠폰" | "예약" | "구매" | "상담";
...
interface IntentType {
  ...
  is_active: boolean;
  purpose?: DbDropPurpose | null;
  default_required_blocks?: BlockKind[] | null;
  allowed_blocks?: BlockKind[] | null;
  requires_disclosure?: boolean | null;
}

// WHY: 9 intent 엔진(DB)은 보존하되 Step 2 UX에는 5목적만 노출 — v3 결정 락 B안.
const PURPOSE_UX_OPTIONS: {
  uxKey: DropPurposeUx;
  label: string;
  description: string;
  dbPurpose: DbDropPurpose;
  /** intent_types.key 우선순위 — 첫 매칭 행이 내부 intent로 저장됨 */
  intentKeys: string[];
  icon: LucideIcon;
  stripVar: string;
...
}[] = [
  {
    ...
    dbPurpose: "정보",
    intentKeys: ["info", "discussion", "campaign", "custom"],
    icon: BookOpen,
    stripVar: "var(--color-intent-info-strip)",
    aiRecommended: true,
  },
  {
    uxKey: "coupon",
    ...
    intentKeys: ["coupon"],
    icon: Gift,
    stripVar: "var(--color-intent-coupon-strip)",
  },
  {
    uxKey: "reservation",
    label: "예약",
    ...
    dbPurpose: "예약",
    intentKeys: ["reservation", "ticket"],
    icon: Calendar,
    stripVar: "var(--color-intent-reservation-strip)",
  },
  {
    uxKey: "purchase",
    label: "구매",
    ...
    intentKeys: ["commerce"],
    icon: ShoppingBag,
    stripVar: "var(--color-intent-commerce-strip)",
  },
  {
    uxKey: "lead",
    label: "상담",
    ...
    intentKeys: ["lead"],
    icon: Phone,
    stripVar: "var(--color-intent-lead-strip)",
  },
];

function resolveIntentForPurpose(uxKey: DropPurposeUx, intents: IntentType[]): IntentType | null {
  const option = PURPOSE_UX_OPTIONS.find((p) => p.uxKey === uxKey);
  if (!option) return null;

  for (const key of option.intentKeys) {
    const byKey = intents.find((i) => i.key === key && i.is_active);
    if (byKey) return byKey;
  }

  const byPurpose = intents.find((i) => i.purpose === option.dbPurpose && i.is_active);
  if (byPurpose) return byPurpose;

  return intents.find((i) => option.intentKeys.includes(i.key)) ?? null;
}

function purposeUxFromIntent(intent: IntentType): DropPurposeUx | null {
  const byOption = PURPOSE_UX_OPTIONS.find((p) => p.intentKeys.includes(intent.key));
  if (byOption) return byOption.uxKey;
  if (intent.purpose) {
    const match = PURPOSE_UX_OPTIONS.find((p) => p.dbPurpose === intent.purpose);
    if (match) return match.uxKey;
  }
  return null;
}
...
/* ---------- Step 2: 5목적 선택 (9 intent 엔진은 내부 매핑만) ---------- */
function Step2({
  selectedIntentId,
  onSelect,
}: {
    ...
    queryFn: async (): Promise<IntentType[]> => {
      ...
      try {
        const { data, error } = await getSupabase()
          .from("intent_types")
          .select(
            "id, key, name, purpose, requires_partner, is_active, default_required_blocks, allowed_blocks, requires_disclosure",
          )
          .eq("is_active", true)
          .order("key");
  ...
  const engineIntents = intents ?? INTENT_FALLBACK;

  const selectedUx = useMemo(() => {
    if (!selectedIntentId) return null;
    const row = engineIntents.find((i) => i.id === selectedIntentId);
    return row ? purposeUxFromIntent(row) : null;
  }, [selectedIntentId, engineIntents]);

  function handlePurposePick(uxKey: DropPurposeUx) {
    const resolved = resolveIntentForPurpose(uxKey, engineIntents);
  ...
  return (
    <main className="flex-1 px-6 pb-12 pt-2">
      ...
      <div className="mt-8 grid grid-cols-2 gap-3">
        {PURPOSE_UX_OPTIONS.map((option) => {
          const Icon = option.icon;
          const isSelected = selectedUx === option.uxKey;
          const resolved = resolveIntentForPurpose(option.uxKey, engineIntents);
          ...
          return (
            <button
              ...
              type="button"
              disabled={disabled}
              onClick={() => handlePurposePick(option.uxKey)}
              className={cn(
                "group relative min-h-[112px] overflow-hidden rounded-2xl border border-border p-4",
            ...
            >
              ...
              <span
                ...
                style={{ background: option.stripVar }}
              />
              <Icon className="size-6 text-text-muted" strokeWidth={2} />
              <div>
                <span className="text-sm font-bold tracking-ko text-text-strong">{option.label}</span>
