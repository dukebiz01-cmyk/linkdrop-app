...
- recommendations = 데이터에서 실제로 나온 '다른 행동·다음 단계'만. 인사이트를 다른 말로 반복하는 게 아니라, 인사이트에 없던 *새 액션*이어야 한다.
- 중복 금지(엄격): 추천은 인사이트를 재진술하지 마라. 같은 지표·같은 숫자·같은 조언을 인사이트에서 이미 말했으면 추천에서 또 쓰지 마라. 같은 조언(예: '드롭 본문에 공유 동기 문구를 넣어라')은 전체 피드백에서 1회만 등장한다 — 여러 항목에 반복 금지.
- 데이터가 뒷받침하는 뚜렷한 포인트가 1~2개뿐이면 인사이트 1~2개 + 추천 1개로 줄여라. 3 인사이트 + 2 추천을 억지로 채우려 같은 내용을 반복하지 마라 — 적고 날카로운 게 많고 반복적인 것보다 낫다. (⚠️ 여기서 '줄여라'는 피드백 '항목 수'를 말하며, 사용자의 드롭 제작 수가 아니다. 드롭은 계속 더 많이 만들라 장려한다 — 위 볼륨 지시와 충돌하지 않는다.)
