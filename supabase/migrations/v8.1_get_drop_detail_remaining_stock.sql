-- v8.1 — get_drop_detail: remaining_stock 파생 필드 추가 (Phase 1-B, A안)
--
-- remaining_stock = 메인 product 블록.stock_limit
--                 − SUM(preorders.quantity WHERE status IN ('pending','confirmed','fulfilled'))
--   · product 블록 존재 + stock_limit 非null 일 때만 계산, 아니면 null.
--   · 메인 product 블록 선정 = create_preorder 와 동일 기준(block_kind='product',
--     ref_drop_id 없음, created_at ASC LIMIT 1) — 쓰기 경로와 같은 블록을 본다(정합).
--   · ⛔ 읽기 전용 파생(L4): 차감 컬럼·트리거 없음. 쓰기 정합은 create_preorder 의
--     advisory lock + SUM 검증이 기보장 — 본 함수는 그 SUM 을 그대로 읽기만 한다.
--   · 기존 반환 필드 전부 불변(최상위 'remaining_stock' 키 추가만) — 기존 소비자 회귀 0.
