-- BOOST1-RESHARE 패치 2 — RETURNS TABLE 변수와 share_events 컬럼명 충돌
-- 사유: RETURNING id, chain_depth, ... 에서 chain_depth/chain_origin_user_id 식별자가
--       RETURNS TABLE 출력 변수와 동명이라 ambiguous. PL/pgSQL 지시문
--       #variable_conflict use_column 한 줄로 컬럼 우선 분기. 본문·시그니처 무변경.
...
DECLARE
  ...
  v_share_uuid        UUID := gen_random_uuid();
