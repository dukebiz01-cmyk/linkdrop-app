...
#### S3 소거 기록 (2026-07-18 — S3-0/1/2/3: /d reservation 수렴 + 결합 정합 + 크롬 정본화 + 카드 내용물 동형화)

닫힘: **결합 판정**(S3-0 — 파트너 캘린더 쿠폰 카드 isCombined 편입, sticky 병존 해소 38건) ·
**예약 본체**(S3-1 — CardModelBody 수렴, initialSlots→카드 내 예약존=스튜디오 cfgDates 거울) ·
**크롬 정본화**(S3-2 — 예약 실행 카드 래핑[캘린더·결제고지·연락 편입]·소박스 토큰 통일·빈 IIFE 제거·
미참조 쿠폰패널 체인 삭제) · **본체 칩**("예약 · 쿠폰 카드" 스튜디오 동형 — 쿠폰존 칩 중복 제거) ·
**카드 내용물 동형**(S3-3 — ③가능일 리스트 결합 편입 ④예약하기/매장정보 버튼[실행=실행카드 스크롤]
⑤시설 태그 관통[공급원 부재=미렌더] ⑥함께 받는 카드 인카드[크롬 관련상품 이중 제거]
⑦내장 공유푸터 거울[라벨 3액션 FIX-55 신스킨 + FTC 인카드, 페이지 하단=법정만]).
동일 카드 스튜디오↔수신 DOM 대조 11항목 전부 동형 확인.
신규 등재: **FIX-57 — cfgTitle 미저장 = 스튜디오 거짓 미리보기**: 발행 payload(비커머스)가 제목을
저장하지 않아 수신 제목=source.title(영상 원제)이 정본인데, 스튜디오 미리보기는 매장명 fallback
(titleFallback: storeName)을 표시 — 발행되지 않는 값의 미리보기(거울 위반, 스튜디오측). 수정안:
fallback을 영상 제목으로 교체 or cfgTitle 발행 저장 — Duke 확정 대기.
**잔존 = S4(purchase 수렴)·S5(구 CardBody/DropCardShell 경로 장례)만 남음.**
...
| G3 | 예약 variant: 인트로 헤더·CTA 색 상이·캘린더 크롬 경계 (본체만 교체, 캘린더 존치 원칙 준수) | **S3 (reservation)** — ✅ 수렴 완료(§1-2 S3 소거 기록). 신규 FIX-57(cfgTitle 미저장) 등재 |
