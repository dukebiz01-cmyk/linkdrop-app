...
import { MeIcon } from "./bottom-nav-v4";
...
function SettingsSheet({
  ...
  const groups: {
    title: string;
    rows: { icon: typeof Bell; label: string; sub?: string; onClick?: () => void }[];
  }[] = [
    {
      title: "비즈니스",
      rows: [{ icon: Store, label: "스토어 관리", sub: "매장 등록·수정", onClick: () => go(onStore) }],
    },
    {
      title: "환경설정",
      rows: [
        { icon: Globe, label: "언어", sub: "한국어", onClick: () => go(onLanguage) },
        { icon: Palette, label: "테마", sub: "라이트", onClick: () => go(onTheme) },
      ],
    },
    {
      title: "계정",
      rows: [
        { icon: Bell, label: "알림 설정", sub: "푸시·이메일", onClick: () => go(onNotifications) },
        { icon: ShieldCheck, label: "개인정보 보호", sub: "보안·데이터", onClick: () => go(onPrivacy) },
  ...
  return createPortal(
    ...
    >
      ...
      <div
        className={`relative mx-auto w-full max-w-md rounded-t-3xl bg-white px-5 pb-8 pt-3 shadow-[0_-8px_40px_rgba(15,23,42,0.24)] transition-transform duration-300 ease-out ${
        ...
        <div className="flex flex-col gap-4">
          {groups.map((group) => (
            <div key={group.title} className="flex flex-col">
              <p className="mb-1 px-1 text-[11px] font-bold uppercase tracking-[0.08em] text-[#94A3B8]">
                {group.title}
              </p>
              {group.rows.map((row) => (
                <button
                ...
                >
                  ...
                  <span className="flex items-center gap-1.5">
                    {row.sub && <span className="text-[12.5px] font-medium text-[#94A3B8]">{row.sub}</span>}
...
export function MeScreen({
  isBusiness = false,
  profile = { name: "", email: "" }, // TODO: get_my_wallet/profiles
  coupons = [], // TODO: get_my_wallet
  dropyBalance = 0, // TODO: get_my_wallet
  dropyTxns = [], // TODO: get_dropy_transactions
  cashBalance = 0, // TODO: get_my_wallet
  cashTxns = [], // TODO: get_cash_transactions
  sentCount = 0, // TODO: get_my_shares
  subscribedMakers = [], // TODO: maker_follows
  myDrops = [], // TODO: get_my_drops
  onEditProfile,
  onLogout,
  onViewMyCards,
  onOpenBusiness,
    ...
    <div className="min-h-screen bg-[#F6F8FB] px-4 pb-24">
      <header className="flex items-center justify-between pb-4 pt-5">
        <div className="flex items-center gap-2.5">
          <span className="flex size-9 items-center justify-center rounded-xl bg-[#F1F5F9]" aria-hidden="true">
            <MeIcon active className="h-[22px] w-[22px]" />
          </span>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className={`grid gap-3 ${isBusiness ? "grid-cols-2" : "grid-cols-1"}`}>
          {isBusiness ? (
            <>
              <NavCard icon={Building2} title="내 매장" subtitle="매출·프로모션·예약" onClick={onOpenBusiness} />
              <NavCard icon={Package} title="판매관리" subtitle="상품·주문 관리" onClick={onOpenSales} />
            </>
          ) : (
        ...
        {/* ⑤ Dropy Mall — 드로피로 구매하는 쇼핑몰 */}
        <button
          onClick={onOpenMall}
          className="group relative flex w-full items-center gap-3.5 overflow-hidden rounded-2xl bg-[#0F172A] px-4 py-4 text-left shadow-[0_12px_30px_rgba(15,23,42,0.22)] transition-transform duration-150 active:scale-[0.99]"
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="flex items-center gap-1.5">
              <span className="text-[15px] font-extrabold tracking-[-0.01em] text-white">Dropy Mall</span>
              <span className="rounded-full bg-[#2563EB] px-1.5 py-0.5 text-[10px] font-bold text-white">NEW</span>
            </span>
          ...
          <span className="flex flex-shrink-0 items-center gap-1 rounded-full bg-white/10 px-2.5 py-1.5">
            ...
            <span className="text-[12px] font-bold tabular-nums text-white">{dropyBalance.toLocaleString()}</span>
          </span>
        </button>
