...
// 배송 진행 단계 — store-hub의 배송 상태와 일관
export const SHIP_STAGES = ["배송준비", "배송중", "배송완료"] as const;

/**
 * CardModel — 제작·공유·수신에서 동일하게 렌더되는 카드의 완성 상태.
 * 모든 표시 문자열은 빌더에서 미리 확정해 넣어, 어느 화면에서 그려도 결과가 같다(거울).
 */
...
type Variant = "studio" | "share" | "receiver";
...
export function CardBody({
  ...
  // 행동 버튼 조립 (본체/배경/강화/쿠폰 제외 — 쿠폰은 아래 도킹 카드로 분리)
  const bodyButtons: {
    id: string;
    label: string;
    icon: LucideIcon;
  ...
  if (applied["party"]) bodyButtons.push({ id: "party", label: `${model.party}명`, icon: Users });
      ...
      >
        ...
        <div className="relative">
          ...
          <div className="flex items-center justify-between">
            ...
            >
              <model.categoryIcon className="h-3.5 w-3.5" strokeWidth={2.5} />
              {model.category}
            </span>
          ...
          {/* 히어로 미디어 */}
          {hasHeroMedia ? (
            <div className="relative mt-3 aspect-video overflow-hidden rounded-2xl bg-[#F4F4F5] ring-1 ring-[#EDEDED] animate-scale-in">
          ...
          {/* 제목 / 설명 */}
          <h3 className="mt-4 text-[21px] font-bold leading-snug tracking-[-0.02em] text-[#0A0A0A] text-balance">
            {model.titleText}
          </h3>
          <p className="mt-1 text-[13px] leading-relaxed text-[#525252] text-pretty">{model.subtitleText}</p>

          {/* 브랜드 소개 */}
          {applied["brand"] && (
            <div
              className="mt-3 flex items-start gap-2.5 rounded-xl py-2 pl-3 pr-3 animate-slide-up"
              style={{ backgroundColor: `${accent}0A`, boxShadow: `inset 3px 0 0 ${accent}` }}
            >
              <Store className="mt-0.5 h-3.5 w-3.5 shrink-0" strokeWidth={2.25} style={{ color: accent }} />
              <p className="text-[12px] font-medium leading-relaxed text-[#525252] text-pretty">
                {model.brandText}
              </p>
            </div>
          )}
          ...
          {applied["product"] && (
            <div className="mt-4 animate-slide-up">
              ...
              >
                <span className="flex items-center gap-1.5 text-[12px] font-semibold" style={{ color: accent }}>
                  <Tag className="h-4 w-4" strokeWidth={2.25} />
                  판매가
                </span>
                <span className="flex items-baseline gap-1.5">
                  {model.productQty && (
                    <span className="text-[10px] font-semibold text-[#8A8A8A]">한정 {model.productQty}개</span>
                  )}
              ...
              {model.productPoints && model.productPoints.length > 0 && (
                <ul className="mt-2 space-y-1">
                  {model.productPoints.map((pt, i) => (
                    <li key={i} className="flex items-start gap-1.5 text-[12px] leading-relaxed text-[#525252]">
                      <Check className="mt-0.5 h-3.5 w-3.5 flex-none" style={{ color: accent }} strokeWidth={2.75} />
          ...
          {/* 배송 안내 — 배송 진행 추적 + 택배사 */}
          {applied["delivery"] && (
            <div className="mt-3 rounded-2xl px-3.5 py-3.5 animate-slide-up" style={{ backgroundColor: `${accent}0A` }}>
              {/* 헤더: 택배사 · 현재 상태 */}
                      ...
                      <div className="flex w-full items-center">
                        ...
                        >
                          {i < stage ? (
                            ...
                            <span className="h-2 w-2 rounded-full" style={{ backgroundColor: `${accent}40` }} />
                          )}
                        </span>
              ...
              <div className="mt-2.5 grid grid-cols-2 gap-1.5">
                <div className="rounded-xl bg-white px-3 py-2">
                  ...
                  <span className="mt-0.5 block text-[12px] font-bold text-[#0A0A0A]">{model.shipFee}</span>
                </div>
                <div className="rounded-xl bg-white px-3 py-2">
                  <span className="flex items-center gap-1 text-[10px] font-semibold text-[#6E6E6E]">
                  ...
                  <span className="mt-0.5 block text-[12px] font-bold text-[#0A0A0A]">{model.shipEta}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {/* 고객 후기 */}
          {applied["review"] && (
            <div className="mt-3 rounded-2xl px-3.5 py-3 animate-slide-up" style={{ backgroundColor: `${accent}0A` }}>
              <div className="flex items-center gap-1.5">
                <div className="flex items-center gap-0.5">
                  {[1, 2, 3, 4, 5].map((n) => (
                    <Star
                      ...
                      strokeWidth={2}
                      style={{
                        color: n <= model.rating ? accent : "#D4D4D4",
                        fill: n <= model.rating ? accent : "transparent",
                      }}
                    />
                  ))}
                </div>
                <span className="text-[12px] font-bold tabular-nums text-[#0A0A0A]">{model.rating.toFixed(1)}</span>
                <span className="ml-auto text-[10px] font-semibold text-[#6E6E6E]">실구매 고객 후기</span>
              </div>
              <p className="mt-1.5 text-[12px] font-medium leading-relaxed text-[#525252] text-pretty">
                {model.reviewText}
              </p>
            </div>
          )}

          {/* 예약 가능일 — 날짜 · 시간대 · 잔여 자리 (많으면 접기) */}
          {applied["calendar"] && <ReservationPreview model={model} accent={accent} />}
            ...
            <div className="mt-3.5 flex flex-wrap items-center gap-1.5">
              {bodyButtons.map((b) => {
                ...
                return (
                  <button
                    key={b.id}
                    className="inline-flex h-9 items-center gap-1.5 rounded-xl px-3 text-[13px] font-semibold animate-slide-up transition-transform duration-150 active:translate-y-px"
                    style={
                      b.main
                        ? {
            ...
            <div className="mt-3 animate-slide-up">
              ...
              >
                ...
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-[13px] font-bold text-[#0A0A0A]">{model.dockTitle}</span>
          ...
          {showJourney && (
            <div className="mt-3.5 border-t border-[#F0F0F0] pt-1">
              ...
              <div className="grid transition-all duration-300 ease-out" style={{ gridTemplateRows: journeyOpen ? "1fr" : "0fr" }}>
                <div className="overflow-hidden">
                  <div className="mt-1 space-y-1 px-0.5 pb-0.5">
                    {model.journey.map((node, i) => {
                      const isLast = i === model.journey.length - 1;
                        ...
                        <div key={i} className="flex items-stretch gap-2.5">
                          <div className="flex flex-none flex-col items-center">
                            <span
                              ...
                              style={
                                isMe
                                  ? { backgroundColor: accent, color: "#fff", boxShadow: `0 4px 10px -3px ${accent}80` }
                                  : isBuyer
                          ...
                          <div
                            ...
                            style={{
                              backgroundColor: isMe ? `${accent}0F` : "transparent",
                            }}
                          >
                            <span
                  ...
                  <div className="relative mt-2 overflow-hidden rounded-xl px-3 py-3" style={{ backgroundColor: "#0F172A" }}>
                    {/* 선명한 상단 액센트 라인 (번짐 없음) */}
                    <span
                    ...
                    <p className="relative mt-2 border-t border-white/10 pt-2 text-[11px] font-medium leading-snug text-white/75">
                      내가 이어준 카드가 <b className="font-bold text-white">{model.spreadCount}명</b>에게 닿았어요
                    </p>
      ...
      {applied["coupon"] && (
        <div className="relative">
          ...
          <div className="relative h-5">
            ...
            <span className="absolute -left-1.5 top-1/2 h-4 w-4 -translate-y-1/2 rounded-full" style={{ backgroundColor: pageBg }} />
            <span className="absolute -right-1.5 top-1/2 h-4 w-4 -translate-y-1/2 rounded-full" style={{ backgroundColor: pageBg }} />
            <div className="absolute left-6 right-6 top-1/2 -translate-y-1/2 border-t-2 border-dashed" style={{ borderColor: `${accent}33` }} />
          </div>

          <div
            className="relative -mt-px w-full overflow-hidden rounded-[26px] p-5 text-[#0A0A0A]"
            style={{ backgroundColor: cardColor, boxShadow: "0 22px 60px -18px rgba(15,23,42,0.14), 0 0 0 1px #EDEDED" }}
          >
            <div className="flex items-center gap-1.5">
              <span
                className="inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-bold"
            ...
            <div className="mt-3 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-[12px] text-[#6E6E6E]">{model.store} · 받는 즉시 사용 가능</p>
              </div>
            </div>
...
/**
 * 예약 가능일 미리보기 — 날짜가 많으면 접어서 카드 길이를 제어한다.
...
function ReservationPreview({ model, accent }: { model: CardModel; accent: string }) {
  ...
  const dates = model.dates && model.dates.length ? model.dates : [model.date];
  ...
  return (
    <div className="mt-3.5 animate-slide-up rounded-2xl border border-[#EDEDED] bg-[#FAFAFA] p-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-semibold text-[#8A8A8A] tabular-nums">
